...
// {
//  opened_by: number - represents who owns it, 0 meaning no one owns it yet
...
module.exports = function logMaze(maze) {
  ...
  maze.forEach((row, i) => {
    ...
    row.forEach((item) => {
      ...
      } else if (item.opened_by === NO_ONE) {
        buffer += "%c•• ";
        colors.push("color: lightgray");
      } else {
        buffer +=
          "%c" + (item.length >= 10 ? item.length : "0" + item.length) + " ";
        colors.push(item.opened_by === BY_A ? "color: lime" : "color: hotpink");
      }
    });
